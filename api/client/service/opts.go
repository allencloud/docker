...
func (m *MountOpt) Set(value string) error {
	...
	for _, field := range fields {
		...
		case "volume-nocopy":
			...
			if err != nil {
				return fmt.Errorf("无效的暴露值: %s", value)
			}
		case "volume-label":
		...
		default:
			return fmt.Errorf("不期望的值 '%s' 在 '%s'", key, field)
		}
	}
	...
	if mount.Type == mounttypes.TypeBind && mount.VolumeOptions != nil {
		return fmt.Errorf("不能将 'volume-*' 选项和挂载类型 '%s' 混用", mounttypes.TypeBind)
	}
	if mount.Type == mounttypes.TypeVolume && mount.BindOptions != nil {
		return fmt.Errorf("不能将 'bind-*' 选项和挂载类型 '%s' 混用", mounttypes.TypeVolume)
	}
...
func (opts *serviceOptions) ToService() (swarm.ServiceSpec, error) {
	...
	default:
		return service, fmt.Errorf("未知的服务编排模式: %s", opts.mode)
	}
	return service, nil
}
...
func addServiceFlags(cmd *cobra.Command, opts *serviceOptions) {
	...
	flags.StringVarP(&opts.workdir, flagWorkdir, "w", "", "容器内部的工作目录")
	flags.StringVarP(&opts.user, flagUser, "u", "", "用户名UID (格式: <用户名|用户名ID>[:<组|组ID>])")
	flags.StringSliceVar(&opts.groups, flagGroupAdd, []string{}, "为容器添加额外的用户组")
	...
	flags.Var(&opts.replicas, flagReplicas, "容器内任务数量")
